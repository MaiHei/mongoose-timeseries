...
var TimeSeriesSchema = function (args) {
    ...
    var options = {
        ...
        roundDay = function (d) {
            return moment({year: d.get('year'), month: d.get('month'), day: d.get('date')});
        },
        schema = new Schema({
    ...
    schema.method('getData', function (interval, format) {
        ...
        var data = [],
            year = this.day.get('year'),
            month = this.day.get('month'),
            day = this.day.get('date'),
            timestamp,
                    ...
                    for (second in this.milliseconds[hour][minute]) {
                        ...
                        for (ms in this.milliseconds[hour][minute][second]) {
                            ...
                            if (this.milliseconds[hour][minute][second][ms]) {
                                ...
                                data.push(dataFormat(timestamp, this.milliseconds[hour][minute][second][ms], format));
                            }
                        }
                    }
                }
    ...
    schema.method('recalc', function (timestamp, value, cb) {
        ...
        if (this.metadata.interval < 1 && this.millisecond) {
                ...
                for (min in this.millisecond[hour]) {
                    ...
                    for (sec in this.millisecond[hour][min]) {
                        ...
                        for (ms in this.millisecond[hour][min][sec]) {
                            ...
                            if (isNaN(parseInt(this.millisecond[hour][min][sec][ms].value))) {
                                continue;
                            }
            ...
            updates['seconds.' + timestamp.get('hour') + '.' + timestamp.get('minute') + '.value'] = sum / i;
            this.set(updates);
        }
        if (this.metadata.interval < 60 && this.seconds) {
            ...
            updates['minutes.' + timestamp.get('hour') + '.' + timestamp.get('minute')] = {value: sum / i};
            this.set(updates);
        }
        if (this.metadata.interval < 3600 && this.minutes) {
            ...
            for (hour in this.minutes) {
                ...
                for (min in this.minutes[hour]) {
                    if (isNaN(parseInt(min))) {
                        break;
                    }
                    if (!this.minutes[hour][min]) {
                        continue;
                    }
                    if (isNaN(parseInt(this.minutes[hour][min].value))) {
                        continue;
                    }
            ...
            updates['hourly.' + timestamp.get('hour') ] = { value: sum / i };
            this.set(updates);
        }
        if (this.metadata.interval >= 3600 && this.hourly) {
            ...
            for (hour in this.hourly) {
                if (this.hourly[hour] === null || isNaN(parseInt(hour))) {
                    break;
                }
                if (!this.hourly[hour]) {
                    continue;
                }
                if (isNaN(parseInt(this.hourly[hour].value))) {
                    continue;
                }
