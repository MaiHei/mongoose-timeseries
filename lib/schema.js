...
var TimeSeriesSchema = function (args) {
    ...
    function getUpdates(timestamp, value, first) {
        ...
        // Format incoming value to array
        if(typeof(value) === 'number') {
            value = [value];
        }

        // Calculate statistics for stage
        var vals = {
            count: value.length,
            sum: _.reduce(value, (total, num) => {return total + num;}, 0),
            sumsquare: _.reduce(value, (total, num) => {return total + (num*num);}, 0),
            min: _.min(value),
            max: _.max(value)
        };
        if (options.interval < 1) {
            ...
            inc[minutePath + '.count'] = vals.count;
            inc[minutePath + '.sum'] = vals.sum;
            inc[minutePath + '.sumSquare'] = vals.sumsquare;
            min[minutePath + '.min'] = vals.min;
            max[minutePath + '.max'] = vals.max;
        } 
        
        if (options.interval > 0) {
            inc['hourly.' + timestamp.get('hour') + '.count'] = vals.count;
            inc['hourly.' + timestamp.get('hour') + '.sum'] = vals.sum;
            inc['hourly.' + timestamp.get('hour') + '.sumSquare'] = vals.sumsquare;
            min['hourly.' + timestamp.get('hour') + '.min'] = vals.min;
            max['hourly.' + timestamp.get('hour') + '.max'] = vals.max;
        }

        //statistics
        updates['updatedAt.date'] = moment.utc().toDate();
        updates['latest.timestamp'] = timestamp.toDate();
        updates['latest.value'] = vals.sum/vals.count;


        inc['statistics.count'] = 1;
        inc['statistics.sum'] = vals.sum;
        inc['statistics.sumSquare'] = vals.sumsquare;
        updates['$inc'] = inc;
        ...
        min['statistics.min'] = vals.min;
        updates['$min'] = min;

        max['statistics.max'] = vals.max;
        updates['$max'] = max;
